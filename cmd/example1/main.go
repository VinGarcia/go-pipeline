...
func main() {
	...
	p.Debug = true
	pipeline := p.New(
		p.NewStage("coolNameForStage1", threads.Num(1), func(_ interface{}) (interface{}, error) {
		...
		p.NewStage("coolNameForStage3", threads.Num(1), func(job interface{}) (interface{}, error) {
		...
		p.NewFanoutStage("myFanoutStage", threads.Num(1),
		...
		p.NewStage(
			// Since this is a very slow stage we can add extra number of tasks:
